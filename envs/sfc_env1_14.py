...
#环境智能体交互 reset step step_low_level step_high_level get_state
    ...
    def step_low_level(self, action):
        ...
        if self._step_count_this_branch > 50:
            logger.error(f"❌ 分支步数超过50，判定为无路可走")
            self._step_count_this_branch = 0
            self.current_branch_id = None  # 退出分支

            return self.get_state(), -100.0, False, False, {
                'failure_reason': 'too_many_steps'
            }

        # 检查2：action_mask全0
        action_mask = self.get_low_level_action_mask()
        if action_mask is not None:
            if (action_mask > 0.1).sum() == 0:
                logger.error(f"❌ 无可行动作，判定为无路可走")
                self._step_count_this_branch = 0
                self.current_branch_id = None
...
#最终树减枝
    ...
    # 🔧 修复后的 _try_reserve_resources 方法
    ...
    # ========================================
    # 🔧 可选：增强版 _finalize_request_with_pruning
    # ========================================
